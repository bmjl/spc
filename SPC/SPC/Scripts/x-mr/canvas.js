...
function X() {
    ...
    var hight = 60; //每行的高度
    var width = 40; //每列的宽度
    Draw(contextX, Max, interval, arr1, ii, jj, Min, width, hight, 'x');
}
...
function S() {
    ...
    var hight = 80; //每行的高度
    var width = 40; //每列的宽度
    Draw(contextR, Max, interval, arr1, ii, jj, Min, width, hight, 's');
}

function Draw(cxt, Max, interval, arr1, ii, jj, min, width, hight, type) {
    ...
    for (var k = 0; k < arr1.length; k++) {
        ...
        //划线
        if (k == 0 || (k == arr1.length - 1 && type == 'x')) {} else if (k == 1 && type == 's') {} else {
            //画点
